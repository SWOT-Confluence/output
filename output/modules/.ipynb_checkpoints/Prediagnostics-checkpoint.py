...
#ORIGINAL 
    ...
    #                 pre_dict['node'][a_variable][i] = pre_ds['node'][a_variable][:,j].filled(self.FILL["i4"])
        ...
        # Get number of nodes from the shape of a variable instead of dimensions
        if len(pre_ds['node'].variables) == 0:
            self.logger.warning('No variables in node group')
            return

        # Get first variable and extract num_nodes from its shape
        first_var_name = list(pre_ds['node'].variables.keys())[0]
        first_var = pre_ds['node'][first_var_name]
        # Variables are shaped (time_steps, num_nodes), so num_nodes is index 1
        num_nodes_in_file = first_var.shape[1]
